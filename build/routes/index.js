...
var UsersController = class {
  ...
  async showTasks(request, response, _) {
    const id2 = import_zod3.default.string().parse(request.params.id);
    if (!request.user || request.user.role !== import_client2.Roles.admin && id2 !== request.user.id) {
...
var TasksController = class {
  ...
  async show(request, response, _) {
    const id2 = import_zod4.z.string().parse(request.params.id);
...
var TeamsController = class {
  ...
  async showTasks(request, response, _) {
    const id2 = import_zod5.default.string().parse(request.params.id);
  ...
  async removeMember(request, response, _) {
    ...
    const { id: id2, user_id } = paramsSchema.parse(request.params);
