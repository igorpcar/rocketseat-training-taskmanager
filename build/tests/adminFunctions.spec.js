...
var UsersController = class {
  ...
  async showTasks(request2, response, _) {
    const id = import_zod3.default.string().parse(request2.params.id);
    if (!request2.user || request2.user.role !== import_client2.Roles.admin && id !== request2.user.id) {
...
var TasksController = class {
  ...
  async show(request2, response, _) {
    const id = import_zod4.z.string().parse(request2.params.id);
...
var TeamsController = class {
  ...
  async showTasks(request2, response, _) {
    const id = import_zod5.default.string().parse(request2.params.id);
  ...
  async removeMember(request2, response, _) {
    ...
    const { id, user_id } = paramsSchema.parse(request2.params);
